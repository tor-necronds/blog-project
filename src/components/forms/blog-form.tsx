...
export function BlogFormFields({
    ...
    const renderError = (field: keyof TouchedFields) => {
        if (touched[field] && clientErrors?.errors?.find((err) => err.path.includes(field))) {
            return (
                <Label className="text-red-500">
                    {clientErrors.errors?.find((err) => err.path.includes(field))?.message}
                </Label>
            )
        }
        return null
                    ...
                    <Popover>
                        <PopoverTrigger asChild>
                            ...
                            >
                                ...
                                {fields.date ? format(fields.date, "PPP") : <span>Pick a date</span>}
                            </Button>
                        </PopoverTrigger>
